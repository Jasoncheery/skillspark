  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-primary-50 to-secondary-50 py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-4xl mx-auto">
        <Link href="/ai-tools" className="text-primary-600 hover:text-primary-700 mb-6 inline-block">
          ← Back to AI Tools
        </Link>
        ...
        <div className="bg-white rounded-xl shadow-lg p-8 mb-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
              ...
              <ul className="space-y-2">
                {tool.features.map((feature: string, index: number) => (
                  <li key={index} className="flex items-start">
                    <svg className="w-5 h-5 mr-2 text-success-500 mt-0.5" fill="currentColor" viewBox="0 0 20 20">
            ...
            <div>
              ...
              <p className="text-lg text-primary-600 font-semibold mb-6">{tool.pricing}</p>

              <h2 className="text-2xl font-bold text-neutral-900 mb-4">Use Cases</h2>
              <ul className="space-y-2">
                {tool.useCases.map((useCase: string, index: number) => (
                  <li key={index} className="flex items-start">
                    <svg className="w-5 h-5 mr-2 text-accent-500 mt-0.5" fill="currentColor" viewBox="0 0 20 20">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div className="bg-success-50 rounded-lg p-6">
              <h2 className="text-xl font-bold text-success-900 mb-4">Pros</h2>
              <ul className="space-y-2">
                {tool.pros.map((pro: string, index: number) => (
                  <li key={index} className="flex items-start">
                    <span className="text-success-600 mr-2">+</span>
                    <span className="text-success-800">{pro}</span>
                  </li>
                ))}
              </ul>
            </div>
            ...
            <div className="bg-error-50 rounded-lg p-6">
              <h2 className="text-xl font-bold text-error-900 mb-4">Cons</h2>
              <ul className="space-y-2">
                {tool.cons.map((con: string, index: number) => (
                  <li key={index} className="flex items-start">
                    <span className="text-error-600 mr-2">-</span>
                    <span className="text-error-800">{con}</span>
                  </li>
                ))}
              </ul>
            </div>
